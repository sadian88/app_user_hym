...
Future<void> saveOneSignalPlayerId() async {
  OneSignal.shared.setSubscriptionObserver((OSSubscriptionStateChanges changes) async {
    log("======================== ONESIGNAL OBSERVER =========================");
    if (changes.to.userId.validate().isNotEmpty && appStore.playerId != changes.to.userId) {
      await appStore.setPlayerId(changes.to.userId.validate());
      updatePlayerId();
    }
    log('notification player id ' + changes.to.toString());
  });
}
...
Future<FirebaseRemoteConfig> setupFirebaseRemoteConfig() async {
  ...
  if (remoteConfig.getString(USER_CHANGE_LOG).isNotEmpty) await setValue(USER_CHANGE_LOG, remoteConfig.getString(USER_CHANGE_LOG));

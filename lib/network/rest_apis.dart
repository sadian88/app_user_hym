...
Future<ConfigurationResponse> getAppConfigurations({bool isCurrentLocation = false, double? lat, double? long}) async {
  try {
    ...
    data.configurations.validate().forEach((data) {
      ...
      } else if (data.key == ONESIGNAL_APP_ID_PROVIDER) {
        if (data.value.validate().isNotEmpty) {
...
Future<DashboardResponse> userDashboard({bool isCurrentLocation = false, double? lat, double? long}) async {
  ...
  try {
    ...
    if (_dashboardResponse.generalSetting != null) {
      if (_dashboardResponse.generalSetting!.siteDescription.validate().isNotEmpty) setValue(SITE_DESCRIPTION, _dashboardResponse.generalSetting!.siteDescription.validate());
      if (_dashboardResponse.generalSetting!.siteCopyright.validate().isNotEmpty) setValue(SITE_COPYRIGHT, _dashboardResponse.generalSetting!.siteCopyright.validate());
      if (_dashboardResponse.generalSetting!.facebookUrl.validate().isNotEmpty) setValue(FACEBOOK_URL, _dashboardResponse.generalSetting!.facebookUrl.validate());
      if (_dashboardResponse.generalSetting!.instagramUrl.validate().isNotEmpty) setValue(INSTAGRAM_URL, _dashboardResponse.generalSetting!.instagramUrl.validate());
      if (_dashboardResponse.generalSetting!.twitterUrl.validate().isNotEmpty) setValue(TWITTER_URL, _dashboardResponse.generalSetting!.twitterUrl.validate());
      if (_dashboardResponse.generalSetting!.linkedinUrl.validate().isNotEmpty) setValue(LINKEDIN_URL, _dashboardResponse.generalSetting!.linkedinUrl.validate());
      if (_dashboardResponse.generalSetting!.youtubeUrl.validate().isNotEmpty) setValue(YOUTUBE_URL, _dashboardResponse.generalSetting!.youtubeUrl.validate());
